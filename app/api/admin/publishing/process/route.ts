...
export async function POST(req: Request) {
  try {
    ...
    const { rows: jobs } = await query<any>(`SELECT pj.*, row_to_json(ga) AS asset, json_build_object('title', c.title, 'images', COALESCE((SELECT json_agg(ia) FROM macula.macula_image_assets ia WHERE ia."caseId" = c.id), '[]')) AS case FROM macula.macula_publication_jobs pj JOIN macula.macula_generated_assets ga ON ga.id = pj."assetId" JOIN macula.macula_cases c ON c.id = pj."caseId" WHERE ${jobFilters.join(" AND ")} ORDER BY pj."createdAt" ASC LIMIT ${body.jobId ? 1 : 25}`, jobValues);
    const connectionPairs = [...new Map(jobs.map((job) => [`${job.organizationId}:${job.platform}`, [job.organizationId, job.platform]])).values()];
    const connections = connectionPairs.length ? (await query<any>(`SELECT * FROM macula.macula_publication_connections WHERE "isActive" = TRUE AND (${connectionPairs.map((_, index) => `("organizationId" = $${index * 2 + 1} AND platform = $${index * 2 + 2})`).join(" OR ")})`, connectionPairs.flat())).rows : [];
    ...
    const processJob = async (job: typeof jobs[number]) => {
      ...
      if (!connection) {
        await query(`UPDATE macula.macula_publication_jobs SET status='FAILED', "failureReason"=$1, "updatedAt"=NOW() WHERE id=$2`, [`No active ${job.platform} connection is configured.`, job.id]);
      ...
      if (connection.expiresAt && connection.expiresAt <= now && connection.refreshTokenEncrypted && ["linkedin", "x", "youtube"].includes(connection.platform)) {
        try {
          ...
          await query(`UPDATE macula.macula_publication_connections SET "accessTokenEncrypted"=$1, "refreshTokenEncrypted"=$2, "expiresAt"=$3, "updatedAt"=NOW() WHERE id=$4`, [refreshed.accessTokenEncrypted, refreshed.refreshTokenEncrypted, refreshed.expiresAt, connection.id]);
          Object.assign(connection, refreshed);
        } catch (error: any) {
          await query(`UPDATE macula.macula_publication_jobs SET status='FAILED', "failureReason"=$1, "updatedAt"=NOW() WHERE id=$2`, [`Token refresh failed: ${error.message}`, job.id]);
          results.push({ id: job.id, status: "FAILED" });
          return;
        }
      }
      await query(`UPDATE macula.macula_publication_jobs SET status='PROCESSING', "lastAttemptAt"=$1, "attemptCount"="attemptCount"+1, "updatedAt"=NOW() WHERE id=$2`, [now, job.id]);
      try {
        ...
        await query(`UPDATE macula.macula_publication_jobs SET status='PUBLISHED', "publishedAt"=$1, "externalId"=$2, "failureReason"=NULL, "nextAttemptAt"=NULL, "updatedAt"=NOW() WHERE id=$3`, [new Date(), published.externalId, job.id]);
        await query(`UPDATE macula.macula_generated_assets SET status='PUBLISHED', "updatedAt"=NOW() WHERE id=$1`, [job.assetId]);
        results.push({ id: job.id, status: "PUBLISHED", externalId: published.externalId });
      } catch (error: any) {
        const attempts = job.attemptCount + 1;
        const retry = attempts < 3;
        await query(`UPDATE macula.macula_publication_jobs SET status=$1, "failureReason"=$2, "nextAttemptAt"=$3, "updatedAt"=NOW() WHERE id=$4`, [retry ? "QUEUED" : "FAILED", error.message || "Publishing failed.", retry ? new Date(Date.now() + attempts * 5 * 60 * 1000) : null, job.id]);
