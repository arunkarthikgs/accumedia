...
export default function SubscriptionPage() {
  ...
  return (
    <main className="readable-route ml-0 mr-auto max-w-5xl space-y-6 p-6 md:p-8">
      ...
      {isLoading ? <div className="rounded-lg border border-line bg-surface p-12 text-center text-sm text-muted"><Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin text-pine" />Loading subscription…</div> : <section className="rounded-lg border border-line bg-surface p-6"><div className="flex items-center gap-2"><CreditCard className="h-5 w-5 text-pine" /><h2 className="text-sm font-bold text-ink">Current plan</h2></div><p className="mt-1 text-xs text-muted">Status: {subscription?.status || "Not configured"}</p><div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2">{plans.map((plan) => <button type="button" key={plan.id} onClick={() => setSelectedPlan(plan.id)} className={`rounded-lg border p-4 text-left ${selectedPlan === plan.id ? "border-pine bg-pine-tint" : "border-line bg-paper"}`}><p className="text-sm font-bold text-ink">{plan.name}</p><p className="mt-2 text-xs text-muted">{plan.monthlyCaseLimit ?? "Unlimited"} cases · {plan.monthlyAudioMinutes ?? "Unlimited"} audio minutes · {plan.monthlyAiTokens ?? "Unlimited"} AI tokens</p></button>)}</div><button type="button" disabled={!selectedPlan || isSaving} onClick={save} className="mt-5 flex items-center gap-2 rounded-lg bg-pine px-4 py-2 text-xs font-semibold text-white disabled:opacity-50"><Save className="h-3.5 w-3.5" />{isSaving ? "Saving…" : "Save plan"}</button>{message && <p className="mt-3 text-xs text-muted">{message}</p>}</section>}
    </main>
  );
}
