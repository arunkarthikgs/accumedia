...
export async function GET(
  ...
  try {
    ...
    const { rows: versions } = await query(`SELECT * FROM macula.case_versions WHERE "caseId" = $1 ORDER BY version DESC`, [id]);
...
export async function PATCH(
  ...
  try {
    ...
    const kase = (await query<any>(`SELECT * FROM macula.cases WHERE id = $1 LIMIT 1`, [id])).rows[0];
    if (!kase) return NextResponse.json({ error: "Case not found." }, { status: 404 });
    await requireOrganizationAccess(kase.organizationId);
    ...
    if (body.action === "restore") {
      const target = (await query<any>(`SELECT * FROM macula.case_versions WHERE id = $1 AND "caseId" = $2 LIMIT 1`, [body.versionId, id])).rows[0];
      const { rows: updatedRows } = await query(`UPDATE macula.cases SET raw_input = $1, "guidedSubmission" = $2::jsonb, "masterRecord" = $3::jsonb, "safetyAudit" = $4::jsonb, status = 'PENDING_REVIEW', mccr_approved_at = NULL, mccr_approved_by = NULL, reviewed_at = NULL, reviewed_by = NULL, "updatedAt" = NOW() WHERE id = $5 RETURNING *`, [target.raw_input, JSON.stringify(target.guidedSubmission), JSON.stringify(target.masterRecord), JSON.stringify(target.safetyAudit), id]);
