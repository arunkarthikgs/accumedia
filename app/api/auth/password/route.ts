...
export async function POST(req: Request) {
  try {
    ...
    const { rows } = await query<{ password_hash: string | null }>(
      `SELECT password_hash FROM macula.users WHERE id = $1 LIMIT 1`,
      [currentUser.id]
    );
    ...
    await query(`UPDATE macula.users SET password_hash = $1 WHERE id = $2`, [await bcrypt.hash(newPassword, 12), currentUser.id]);
    await query(`DELETE FROM macula.sessions WHERE "userId" = $1`, [currentUser.id]);
