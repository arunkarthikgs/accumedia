...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/organizations").then(async (response) => {
      ...
      else { setOrganizationLoadError("No organizations are available for subscription management."); setIsLoading(false); }
    }).catch((error: Error) => { setOrganizationLoadError(error.message); setIsLoading(false); });
  }, []);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (cached) {
          const cachedData = JSON.parse(cached) as { data: any; cachedAt: number };
          if (Date.now() - cachedData.cachedAt < 60_000) {
            ...
            setSelectedPlan(cachedData.data.subscription?.planId || cachedData.data.plans?.[0]?.id || "");
            setIsLoading(false);
          }
        }
      } catch {
      ...
      const data = await (await fetch(`/api/admin/subscription?orgId=${organizationId}`)).json();
      setPlans(data.plans || []);
      setSubscription(data.subscription);
      setQuota(data.quota);
      setSelectedPlan(data.subscription?.planId || data.plans?.[0]?.id || "");
      try { sessionStorage.setItem(cacheKey, JSON.stringify({ data, cachedAt: Date.now() })); } catch { /* Ignore storage limits. */ }
      setIsLoading(false);
    }
  ...
  return (
    <main className="readable-route ml-0 mr-auto max-w-5xl space-y-6 p-6 md:p-8">
      <header>
        ...
        <div><h1 className="text-2xl font-bold tracking-tight text-ink">Subscription &amp; Quotas</h1><p className="mt-0.5 text-xs text-muted">Configure plan limits before billing and overage enforcement are connected.</p></div>
        <div className="flex items-center gap-2">
          <Building2 className="h-4 w-4 text-muted" />
          <label className="sr-only" htmlFor="subscription-organization">Organization</label>
          <select id="subscription-organization" value={organizationId} onChange={(event) => { setIsLoading(true); setOrganizationId(event.target.value); }} disabled={organizations.length === 0} className="rounded border border-line bg-surface px-3 py-1.5 text-xs text-ink focus:border-pine focus:outline-none"><option value="" disabled>{organizations.length ? "Select organization" : "Loading organizations…"}</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}</option>)}</select>
        </div>
        {organizationLoadError && <div className="mt-3 rounded border border-brick/30 bg-brick-tint p-3 text-xs text-brick">{organizationLoadError}</div>}
      </header>
      {!isLoading && plans.length > 0 && <section className="rounded-lg border border-line bg-surface p-6">
        <h2 className="text-sm font-bold text-ink">RFP commercial plans</h2>
        <p className="mt-1 text-xs text-muted">Monthly pricing and quota policy for the selected organisation.</p>
        <div className="mt-4 overflow-x-auto rounded border border-line"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-paper text-[10px] uppercase tracking-wide text-muted"><tr><th className="px-4 py-3">Plan</th><th className="px-4 py-3">Cases/month</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Setup fee</th><th className="px-4 py-3">Overage policy</th></tr></thead><tbody className="divide-y divide-line">{plans.map((plan) => <tr key={`commercial-${plan.id}`}><td className="px-4 py-3 font-semibold text-ink">{plan.name}</td><td className="px-4 py-3 text-muted">{plan.monthlyCaseLimit ?? "Custom"}</td><td className="px-4 py-3 font-semibold text-pine">{plan.isCustom ? "Custom pricing" : `${plan.currency} ${Number(plan.monthlyPrice || 0).toLocaleString()} / ${plan.billingInterval}`}</td><td className="px-4 py-3 text-muted">{plan.setupFee == null ? "Custom" : `${plan.currency} ${Number(plan.setupFee).toLocaleString()}`}</td><td className="px-4 py-3 text-muted">{plan.overagePolicy || "Not configured"}</td></tr>)}</tbody></table></div>
      </section>}
