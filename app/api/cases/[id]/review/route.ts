...
export async function GET(_req: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { rows } = await query<any>(`SELECT c.id, c.title, c.status, c.raw_input AS "rawInput", c."guidedSubmission" AS "guidedSubmission", c."masterRecord" AS "masterRecord", c."safetyAudit" AS "safetyAudit",
      ...
      COALESCE((SELECT json_agg(json_build_object('id', ga.id, 'channelKey', ga."channelKey", 'channelName', ga."channelName", 'outputType', ga."outputType", 'status', ga.status, 'version', ga.version, 'content', ga.content, 'seoMetadata', (SELECT row_to_json(scm) FROM macula.seo_content_metadata scm WHERE scm."assetId" = ga.id), 'validationWarnings', ga."validationWarnings", 'videoR2Key', ga."videoR2Key", 'videoDurationSeconds', ga."videoDurationSeconds", 'videoStatus', ga."videoStatus")) FROM macula.generated_assets ga WHERE ga."caseId" = c.id), '[]') AS assets,
      COALESCE((SELECT json_agg(json_build_object('id', cs.id, 'fileName', cs."fileName", 'sourceType', cs."sourceType", 'status', cs.status, 'processingError', cs."processingError")) FROM macula.case_sources cs WHERE cs."caseId" = c.id), '[]') AS sources
      FROM macula.cases c JOIN macula.users u ON u.id = c."physicianId" JOIN macula.organizations o ON o.id = c."organizationId" WHERE c.id = $1 LIMIT 1`, [id]);
    const kase = rows[0];
