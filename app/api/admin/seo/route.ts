...
export async function GET(req: Request) {
  try {
    ...
    const { rows: sets } = await query(`SELECT sk.*, json_build_object('id', c.id, 'title', c.title, 'status', c.status) AS case FROM macula.macula_seo_keyword_sets sk JOIN macula.macula_cases c ON c.id = sk."caseId" WHERE c."organizationId" = $1 ORDER BY sk."updatedAt" DESC`, [organizationId]);
...
export async function PATCH(req: Request) {
  try {
    ...
    const existing = (await query<any>(`SELECT sk.*, c."organizationId" FROM macula.macula_seo_keyword_sets sk JOIN macula.macula_cases c ON c.id = sk."caseId" WHERE sk.id = $1 LIMIT 1`, [body.id])).rows[0];
    ...
    const duplicate = (await query<any>(`SELECT id, "caseId" FROM macula.macula_seo_keyword_sets WHERE "contentHash" = $1 AND id <> $2 LIMIT 1`, [quality.contentHash, body.id])).rows[0];
