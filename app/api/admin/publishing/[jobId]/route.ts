...
export async function PATCH(req: Request, props: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const job = (await query<any>(`SELECT * FROM macula.publication_jobs WHERE id = $1 LIMIT 1`, [jobId])).rows[0];
    ...
    if (action === "cancel") {
      ...
      const { rows } = await query(`UPDATE macula.publication_jobs SET status = 'CANCELLED', "failureReason" = NULL, "nextAttemptAt" = NULL, "updatedAt" = NOW() WHERE id = $1 RETURNING *`, [jobId]);
      const updated = rows[0];
      return NextResponse.json({ success: true, job: updated });
    }
    if (action === "retry") {
      if (job.status !== "FAILED") return NextResponse.json({ error: "Only failed jobs can be retried." }, { status: 409 });
      const { rows } = await query(`UPDATE macula.publication_jobs SET status = 'QUEUED', "failureReason" = NULL, "nextAttemptAt" = NULL, "updatedAt" = NOW() WHERE id = $1 RETURNING *`, [jobId]);
      const updated = rows[0];
      return NextResponse.json({ success: true, job: updated });
    }
