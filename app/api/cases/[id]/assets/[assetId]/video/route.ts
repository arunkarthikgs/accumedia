...
export async function GET(_req: Request, props: { params: Promise<{ id: string; assetId: string }> }) {
  try {
    ...
    const asset = (await query<any>(`SELECT ga."videoR2Key", c.id AS case_id, c."organizationId" FROM macula.macula_generated_assets ga JOIN macula.macula_cases c ON c.id=ga."caseId" WHERE ga.id=$1 LIMIT 1`, [assetId])).rows[0];
