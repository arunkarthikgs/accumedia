  ...
  try {
    ...
    const organizations: { id: string; name: string; slug: string }[] = user.isSuperAdmin
      ? roleOrganizationsCache && roleOrganizationsCache.expiresAt > Date.now()
        ...
        : (await query<{ id: string; name: string; slug: string }>(`SELECT id, name, slug FROM macula.organizations ORDER BY name ASC`)).rows
      : [];
    ...
    if (roleCatalogCache && roleCatalogCache.expiresAt > Date.now()) {
      ...
      const catalog = await Promise.all([
        query(`SELECT id, name, module, description, slug FROM macula.permissions ORDER BY module ASC, slug ASC`),
        query(`SELECT td.id, td.slug, td.name, td.module, td.description, COALESCE(jsonb_agg(jsonb_build_object('id', p.id)) FILTER (WHERE p.id IS NOT NULL), '[]') AS permissions FROM macula.task_definitions td LEFT JOIN macula.permissions p ON p."taskDefinitionId" = td.id GROUP BY td.id ORDER BY td.module ASC, td.slug ASC`),
      ]);
    ...
    const [{ rows: roles }] = await Promise.all([
      query(`SELECT r.id, r.name, r.slug, r."isSystem" AS "isSystem", r."organizationId" AS "organizationId",
             ...
             FROM macula.roles r
             LEFT JOIN macula.role_definitions rd ON rd.id = r."definitionId"
             LEFT JOIN macula.role_permissions rp ON rp."roleId" = r.id
    ...
    const currentOrganization = organizationId
      ? organizations.find((organization) => organization.id === organizationId) || (await query<{ id: string; name: string; slug: string }>(
        `SELECT id, name, slug FROM macula.organizations WHERE id = $1 LIMIT 1`, [organizationId]
      )).rows[0] || null
      : null;
...
export async function POST(req: Request) {
  try {
    ...
    if (body.action === "createRole") {
      ...
      let slug = baseSlug;
      let suffix = 2;
      while ((await query(`SELECT id FROM macula.roles WHERE slug = $1 AND "organizationId" = $2 LIMIT 1`, [slug, targetOrganizationId])).rows[0]) {
        slug = `${baseSlug}-${suffix++}`;
      }

      const roleId = crypto.randomUUID();
      const { rows: createdRoles } = await query(
        `INSERT INTO macula.roles (id, name, slug, description, "organizationId", "isSystem")
    ...
    const role = (await query<{ "organizationId": string | null; "isSystem": boolean }>(
      `SELECT "organizationId", "isSystem" FROM macula.roles WHERE id = $1 LIMIT 1`, [roleId]
    )).rows[0];
    const permission = (await query(`SELECT id FROM macula.permissions WHERE id = $1 LIMIT 1`, [permissionId])).rows[0];
    ...
    if (enabled) {
      await query(
        `INSERT INTO macula.role_permissions ("roleId", "permissionId")
         VALUES ($1, $2) ON CONFLICT ("roleId", "permissionId") DO NOTHING`,
        [roleId, permissionId]
      );
    } else {
      await query(`DELETE FROM macula.role_permissions WHERE "roleId" = $1 AND "permissionId" = $2`, [roleId, permissionId]);
    }
