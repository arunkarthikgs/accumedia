...
export async function POST(req: Request) {
  try {
    ...
    if (recordingId) {
      await query(`UPDATE macula.audio_recordings SET "transcriptionStatus" = 'REFINING', "updatedAt" = NOW() WHERE id = $1`, [recordingId]);
    }
    ...
    if (unresolvedRules.length > 0) {
      const recording = recordingId ? (await query<{ caseId: string | null }>(`SELECT "caseId" FROM macula.audio_recordings WHERE id = $1 LIMIT 1`, [recordingId])).rows[0] : null;
      if (recording?.caseId) {
        await query(`INSERT INTO macula.safety_flags (id, "targetType", "caseId", "flagType", detail, confidence, status) VALUES ($1, 'CASE', $2, 'pii', $3, 'high', 'OPEN')`, [crypto.randomUUID(), recording.caseId, `LLM refinement blocked: ${unresolvedRules.join("; ")}. Update the database redaction rule before continuing.`]);
      }
      if (recordingId) await query(`UPDATE macula.audio_recordings SET "transcriptionStatus" = 'SAFETY_REVIEW', "updatedAt" = NOW() WHERE id = $1`, [recordingId]);
